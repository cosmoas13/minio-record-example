...
const ListObjects = () => {
	...
	const listObjectsOfBucket = async () => {
		try {
			const objectsStream = mc.extensions.listObjectsV2WithMetadata(
				bucketName,
				'',
				true,
				''
			...
			objectsStream.on('data', async (chunk) => {
				...
				const presignedUrl = await mc.presignedGetObject(
					bucketName,
					objectName,
					24 * 60 * 60
				);
	...
	const openPutObject = async () => {
		...
		if (formValues) {
			const { objectName, objectFile } = formValues;
	...
	return (
		<div className='container'>
			<div>
				<div>
					<span className='mr-3 hover:underline text-gray-400'>
										...
										<tr key={obj.name}>
											...
											<td className='px-6 py-4 whitespace-nowrap'>
												<div className='text-sm text-gray-900'>
													<div className='flex '>
														<div className='mr-2'>{getFileIcon(obj)}</div>
														<div>
															<div className='text-sm font-medium text-gray-900'>
																{obj.name}
															</div>
											...
											<td className='px-6 py-4 whitespace-nowrap'>
												<span className='px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800'>
													{/* {obj.metadata['content-type']} */}File
												</span>
											</td>
											<td className='text-gray-500'>
												{format(obj.lastModified, 'dd MMM yyyy hh:mm a')}
											</td>
											<td className='px-6 py-4 whitespace-nowrap text-right text-sm font-medium'>
												<a
													target='_blank'
													rel='noopener noreferrer'
													href={obj.presignedUrl}
